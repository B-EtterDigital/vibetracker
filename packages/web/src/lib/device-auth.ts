...
export function buildDeviceAuthLaunchTheatre(state: DeviceAuthState, code: string): DeviceAuthLaunchTheatre {
  ...
  const steps: DeviceAuthLaunchStep[] = [
    ...
    },
    {
      ...
      note: "The sync writes a local receipt; records stay on disk until the user chooses an upload command.",
      ...
      transcript: ["local files remain local", "receipt records the sync", "approval uploads nothing"],
    },
    {
      id: "c0vibe-relay",
      phase: "05/05 RELAY",
