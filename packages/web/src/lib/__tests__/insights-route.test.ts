...
test("insights route is a route-local explained cost plan", () => {
  ...
  assert.match(consoleSource, /This example pace lands near/);
  assert.match(consoleSource, /At this pace, @\$\{handle\} lands near/);
  assert.match(consoleSource, /This is teaching data, not your account/);
  assert.match(consoleSource, /days are still outside the evidence window/);
  ...
  assert.match(briefSource, /<InsightEvidenceScope source=\{source\}/);
  ...
  assert.doesNotMatch(styles, /\.intel-decision/);
  ...
  assert.match(briefSource, /The decision, without the dashboard jargon/);
  assert.match(briefSource, /Forecast confidence/);
  assert.match(briefSource, /Do next/);
  assert.match(briefSource, /Plain English/);
  assert.match(briefSource, /estimated paid-provider spend against/);
  assert.match(briefSource, /Highest observed spend\. That makes it the first place to inspect/);
  ...
  assert.match(responsive, /max-width: min\(3200px, 94vw\)/);
  assert.doesNotMatch(responsive, /\.intel-console \{ display: grid/);
