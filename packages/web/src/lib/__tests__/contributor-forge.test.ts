...
test("contributors route renders the launchpad before deeper contributor boards with styling hooks", () => {
  const page = readFileSync(new URL("../../app/contributors/page.tsx", import.meta.url), "utf8");
  const styles = readFileSync(new URL("../../app/globals.css", import.meta.url), "utf8");

  assert.match(page, /ContributorSignalRelayPanel/);
  assert.match(page, /contributorRelayStages/);
  assert.match(page, /contributorRelayFrames/);
  assert.match(page, /className="contributor-relay"/);
  assert.match(page, /CONTRIBUTOR-SIGNAL-RELAY/);
  assert.match(page, /SDK -> fixture -> harness -> publish/);
  assert.match(page, /Vibers Unite \/ c0vibe\.app/);
  assert.match(page, /rank impact: none/);
  assert.match(page, /buildContributorLaunchpad/);
  assert.match(page, /ContributorLaunchpadPanel/);
  assert.match(page, /className="contributor-launchpad"/);
  assert.match(page, /launchpad\.lanes\.map/);
  assert.match(page, /lane\.checklist\.map/);
  assert.match(page, /buildContributorQuestBoard/);
  assert.match(page, /ContributorQuestBoardPanel/);
  assert.match(page, /className="contributor-quest-board"/);
  assert.equal(page.indexOf("<ContributorSignalRelayPanel"), page.lastIndexOf("<ContributorSignalRelayPanel"));
  assert.equal(page.indexOf("<ContributorSignalRelayPanel") < page.indexOf("<ContributorLaunchpadPanel"), true);
  assert.equal(page.indexOf("<ContributorLaunchpadPanel"), page.lastIndexOf("<ContributorLaunchpadPanel"));
  assert.equal(page.indexOf("<ContributorLaunchpadPanel") < page.indexOf("<ContributorQuestBoardPanel"), true);
  assert.equal(page.indexOf("<ContributorQuestBoardPanel"), page.lastIndexOf("<ContributorQuestBoardPanel"));
  assert.equal(page.indexOf("<ContributorQuestBoardPanel") < page.indexOf("<ContributorAdapterFoundryPanel"), true);
