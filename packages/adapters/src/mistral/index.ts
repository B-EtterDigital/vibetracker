...
export function createHttpClient(cfg: { apiKey: string; baseUrl?: string }): UsageClient {
  const base = cfg.baseUrl ?? "https://api.mistral.ai/v1";
  return {
    async usage(range: DateRange): Promise<UsageItem[]> {
      ...
      if (!res.ok) throw new Error(`Mistral usage ${res.status}: ${await res.text()}`);
      const body = await res.json() as { data?: Array<{ date: string; model?: string; tokens?: number; total_tokens?: number; cost?: number }> };
      return (body.data ?? []).map((d) => ({
        ts: new Date(d.date).toISOString(),
        model: d.model,
        amount: d.total_tokens ?? d.tokens ?? 0,
        unit: "token",
        usd: d.cost,
        category: "llm",
      }));
...
export function createMistralAdapter(client: UsageClient) {
  return createUsageAdapter({ id: "mistral", categories: ["llm"], client });
}
