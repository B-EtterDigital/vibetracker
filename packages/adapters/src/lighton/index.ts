...
export function createHttpClient(cfg: { apiKey: string; baseUrl?: string }): UsageClient {
  const base = cfg.baseUrl ?? "https://api.lighton.ai";
  return {
    async usage(range: DateRange): Promise<UsageItem[]> {
      ...
      if (!res.ok) throw new Error(`LightOn usage ${res.status}: ${await res.text()}`);
...
export function createLightOnAdapter(client: UsageClient) {
  return createUsageAdapter({ id: "lighton", categories: ["llm"], client });
}
